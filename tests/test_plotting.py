...
def test_plot_sentences_per_company():
    import pandas as pd

    from esgpillar.plotting import plot_sentences_per_company

    df = pd.DataFrame({"company": ["Acme", "Acme", "Beta", "Gamma", "Gamma", "Gamma"]})
    fig = plot_sentences_per_company(df)
    assert fig is not None
    ax = fig.axes[0]
    assert ax.get_title() == "Frasi per azienda"
    # 3 aziende -> 3 barre, e bar_label aggiunge 3 etichette di testo
