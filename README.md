...
6. **Split train/validation/test** (`esgpillar.split_train_val_test`) —
   divide il dataset in tre insiemi **per azienda** (nessuna azienda
   compare in più di uno), con le tre percentuali a scelta (default
   70/15/15). A differenza dello split interno di `fit()` (solo
   train/validation), questo dà un vero terzo insieme di **test**, da
   tenere fuori da qualunque addestramento per una valutazione finale
   onesta — lo stesso schema usato nei notebook di confronto, qui come
   funzione riusabile.
7. **Reti neurali** (`esgpillar.models`, `esgpillar.training`) — undici
   ...
   **AttentionPool** (attenzione), **MLPMixer** (niente attenzione né
   ...
   learning rate.
8. **Modelli classici** (`esgpillar.classical`) — 24 classificatori
...
9. **Analisi linguistica** (`esgpillar.text_analysis`) — strumenti di
...
10. **Text mining** (`esgpillar.text_mining`) — tecniche a livello di
...
11. **API di alto livello** (`esgpillar.ESGPillarClassifier`) — `fit` /
   `predict` / `save` / `load` in poche righe. `evaluate(sentences, labels)`
   da' un report completo in stile `sklearn.classification_report`:
   precision/recall/F1-score/support per ciascuna classe (`report["Social"]
   ["recall"]`, ecc.), più `accuracy` globale, `"macro avg"` e
   `"weighted avg"`. `network_kwargs` lascia scegliere gli iperparametri
   della rete/del modello (invece dei valori di default della classe) —
   es. `network_kwargs={"h": 256, "p": 0.6}` per una BiLSTM con hidden
   size e dropout personalizzati, o `network_kwargs={"n_estimators": 500}`
   per una Random Forest; per gli stimatori avvolti in
   `CalibratedClassifierCV` (LinearSVM/Ridge/Perceptron) gli override si
   applicano allo stimatore interno, non al wrapper; Voting/Stacking non
   supportano override (combinano più sotto-modelli: non sarebbe chiaro a
   quale si applicherebbero). **`network` accetta anche un'architettura
   costruita a mano**, non solo i nomi predefiniti: una CLASSE
   `torch.nn.Module` per una rete neurale personalizzata (richiede anche
   `custom_input_type="pooled"` o `"sequence"`, per dire alla libreria se
   passarle il vettore mediato per frase o l'intera sequenza di token), o
   un'ISTANZA scikit-learn-compatibile già costruita (`.fit()`/
   `.predict_proba()`) per un modello classico non già tra i 24 nativi —
   `save()`/`load()` funzionano anche con reti/modelli personalizzati.
12. **Tool web** (`esgpillar.tool`) — app Gradio: carica un PDF, ottieni la
   classificazione di ogni frase.
13. **Grafici** (`esgpillar.plotting`) — curve di training (loss + accuracy/F1),
...
# 2bis) split train/val/test per azienda (opzionale: fit() farebbe comunque
# il suo split interno train/val, ma qui hai anche un vero test tenuto fuori)
from esgpillar import split_train_val_test
train_df, val_df, test_df = split_train_val_test(df, train_size=0.7, val_size=0.15, test_size=0.15)

# 3) addestra il classificatore
clf = ESGPillarClassifier(embedding="glove", network="BiLSTM")
clf.fit(train_df, text_col="sentence", company_col="company", label_col="label")

# oppure, con iperparametri personalizzati invece dei valori di default:
# clf = ESGPillarClassifier(embedding="glove", network="BiLSTM", network_kwargs={"h": 256, "p": 0.6})
...
# oppure un'architettura costruita a mano, non un nome predefinito:
# import torch.nn as nn
# class MiaRete(nn.Module):
#     def __init__(self, d, c=3, hidden=128):
#         super().__init__()
#         self.fc1 = nn.Linear(d, hidden)
#         self.fc2 = nn.Linear(hidden, c)
#     def forward(self, x):  # x: vettore pooled, un vettore per frase
#         return self.fc2(torch.relu(self.fc1(x)))
#
# clf = ESGPillarClassifier(
#     embedding="word2vec", network=MiaRete,
#     network_kwargs={"hidden": 256}, custom_input_type="pooled",
# )
